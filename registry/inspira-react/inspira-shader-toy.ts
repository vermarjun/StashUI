...
export class InspiraShaderToy {
  ...
  private setupMouseEvents(): void {
    ...
    const getScaledMousePos = (event: MouseEvent | Touch) => {
      ...
      return {
        ...
        y: ((canvas as HTMLCanvasElement).height - y * dpr) * this._mouseSensitivity,
      };
    };

    canvas.addEventListener("mousemove", (event: Event) => {
      const me = event as MouseEvent;
      const { x: newX, y: newY } = getScaledMousePos(me);
    ...
    canvas.addEventListener("mousedown", (event: Event) => {
      isMouseDown = true;
      const me = event as MouseEvent;
      const { x: clickX, y: clickY } = getScaledMousePos(me);
  ...
  private draw(): void {
    ...
    this.program.uniforms.iMouse.value = [
      this.iMouse.x, this.iMouse.y, this.iMouse.clickX, this.iMouse.clickY,
    ];
