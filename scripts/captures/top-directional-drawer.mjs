/**
 * Choreography: top-directional-drawer
 * Click the Edit button on the profile image to open a top-sliding drawer.
 */
export default async function capture(page, { W, H, wait }) {
  ...
  try {
    ...
    await wait(900); // top drawer slides down
  } catch (_) {}

  // 3. Dwell inside the drawer (top portion of screen)
  try {
    await page.mouse.move(W / 2, H * 0.2, { steps: 10 });
    await wait(600);
    await page.mouse.move(W * 0.4, H * 0.3, { steps: 8 });
    await wait(700);
  } catch (_) {}

  // 4. Close — click overlay at bottom
  try {
