/**
 * Choreography: comp-316
 * Scrollable dialog (custom scrollbar) — trigger: "Scrollable (custom scrollbar)" button
 * Dialog: FAQ content in ScrollArea, Cancel / Okay footer
 */
export default async function capture(page, { W, H, wait }) {
  ...
  try {
    await page.getByRole('button', { name: /scrollable \(custom scrollbar\)/i }).first().click();
    await wait(800);
  } catch (_) {
