/**
 * Capture choreography for comp-451
 * Breadcrumb in a bordered card (rounded border + shadow): HomeIcon → Components → Breadcrumb.
...
export default async function capture(page, { W, H, wait }) {
  ...
  // Dwell on the card-style breadcrumb
  try {
    await page.mouse.move(cx, cy, { steps: 10 });
    await wait(700);
  } catch (_) {}

  // Hover over home icon link
  try {
  ...
  try {
    ...
    try {
      await page.mouse.move(cx - 10, cy, { steps: 8 });
      await wait(700);
    } catch (_) {}
  }
